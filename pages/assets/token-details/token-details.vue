<template>
	<view class="details">
		<u-navbar :title="details.name" height="50px" @rightClick="rightClick" :autoBack="true">
			<u--text slot="right" size="32rpx" bold text="Transfer"></u--text>
		</u-navbar>
		<view class="brief">
			<view class="info">
				<u-avatar class="logo" :size="50" :text="details.symbol[0]" bgColor="#000"></u-avatar>
				<u--text class="text" align="center" size="35rpx" bold :text="details.symbol"></u--text>
			</view>
				...
				<u-cell-group :border="false">
					<u-cell v-for="item,index in holders" :key="index">
						...
						<view slot="right-icon">
							<u--text :text="Number(item.amount).toFixed(4)" align="right" size="33rpx"></u--text>
...
<script>
	export default {
		...
		methods: {
			getToken() {
				uni.request({
					url: this.$market+"/api/token/" + this.details.kid,
					success: (res) => {
						if (res.data.data) {
			...
			getScript() {
				uni.request({
					url: this.$indexer+"/api/script/" + this.details.kid,
					success: (res) => {
						if (res.data.data) {
			...
			getTokenDist() {
				uni.request({
					url: this.$market+"/api/dist/20/" + this.details.kid,
					success: (res) => {
						if (res.data.data) {
			...
			rightClick() {
				uni.navigateTo({
					url: '/pages/assets/token-details/transfer/transfer?kid=' + this.details.kid
				})
			},
